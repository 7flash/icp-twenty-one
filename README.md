...
```
  var volunteers: [var Principal] = [var];
```

Associated with achievement by shared index key

### Create Achievement

Tell about your achievement (good deeds)

```
  public func createAchievement(description: Text) : async Text
```

### Confirm Achievement

Approve validity of another volunteer's achievement

```
  public shared(msg) func confirmAchievement(idx: Nat) : async TokenIndex
```

NFT will be minted for the volunteer

## Example

```
dfx canister create icp_twenty_one
dfx build icp_twenty_one
dfx canister install icp_twenty_one --mode=reinstall
dfx canister call icp_twenty_one createAchievement "first"
